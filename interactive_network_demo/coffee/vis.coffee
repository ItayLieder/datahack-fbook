...
Network = () ->
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      managers = sortedmanagers(curNodesData, curLinksData)
      updateCenters(managers)
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      match = d.skills.toLowerCase().search(searchRegEx)
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.manager))
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.pagerank)
    ...
    data.nodes.forEach (n) ->
      ...
      n.radius = circleRadius(n.pagerank)
  ...
  # attr is value stored in node, like 'manager'
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      pageranks = allNodes.map((d) -> d.pagerank).sort(d3.ascending)
      cutoff = d3.quantile(pageranks, 0.5)
  ...
  # Returns array of managers sorted based on
  # current sorting method.
  sortedmanagers = (nodes,links) ->
    managers = []
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.manager] ?= 0
        counts[l.source.manager] += 1
        counts[l.target.manager] ?= 0
        counts[l.target.manager] += 1
      # add any missing managers that dont have any links
      nodes.forEach (n) ->
        counts[n.manager] ?= 0

      # sort based on counts
      managers = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      managers = managers.map (v) -> v.key
    else
      # sort managers by song count
      counts = nodeCounts(nodes, "manager")
      managers = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      managers = managers.map (v) -> v.key
  ...
  updateCenters = (managers) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(300).increment(18).keys(managers)
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.manager))
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.manager)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.manager)).darker().toString()

  # Mouseover tooltip function
  showDetails = (d,i) ->
    content = '<p class="main">' + d.name + '</span></p>'
    content += '<hr class="tooltip-hr">'
    content += '<p class="main">' + d.skills + '</span></p>'
...
$ ->
  ...
  d3.json "data/Facebook.json", (json) ->
    myNetwork("#vis", json)
